describe('Issue details editing', () => {
  ...
  // Bonus Task 2: Checking Reporter’s Name for Only Characters
it('Should validate reporter’s name contains only characters', () => {
    getIssueDetailsModal().within(() => {
        cy.get('[data-testid="select:reporter"]').invoke('text').then((text) => {
            expect(text.trim()).to.match(/^[A-Za-z\s]+$/);
        });
    });
});
